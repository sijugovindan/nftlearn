...
contract("NFTMarket", accounts => {
  ...
  describe("Mint token", () => {
    ...
    /*it("should resolve into true value", () => {
      ...
      let numberOfNfts = 12;
      assert(numberOfNfts == 12, "Value is not same as expected");
    })*/
    ...
    it("should have create NFT item", async () => {
      const asset = await _contract.getAsset(1);
      console.log("Asset:", asset);
      assert.equal(asset.tokenId, 1, "Token id is not 1");
      assert.equal(asset.creator, accounts[0], "Creator is not account[0]");
      assert.equal(asset.isListed, false, "Token is not listed");
    })
